...
$PREFIX docker buildx build --target=cbtemulator --platform linux/amd64,linux/s390x,linux/arm64,linux/ppc64le --tag fullstorydev/cbtemulator:${VERSION} --tag fullstorydev/cbtemulator:latest --push --progress plain --no-cache .
rm VERSION
